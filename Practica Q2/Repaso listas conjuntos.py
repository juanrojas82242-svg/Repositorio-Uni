...
def validar_subcuadro(tablero, fila_inicio, col_inicio):
    subcuadro = {tablero[i+fila_inicio][j+col_inicio] for i in range(3) for j in range(3)}
    return subcuadro == NUMEROS_VALIDOS

# Ejemplos de prueba
print(validar_fila(TABLERO, 0))       # True
print(validar_columna(TABLERO, 0))    # True
print(validar_subcuadro(TABLERO, 0, 0)) # True
...
def tiene_permisos(permisos_usuario, permisos_requeridos):
    ...
    actual = permisos_requeridos.cabeza
    while actual:
        if not permisos_usuario.pertenece(actual.dato):
            return False
        actual = actual.siguiente
    return True 
    pass
